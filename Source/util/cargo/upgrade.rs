...
fn is_version_dep(dependency:&cargo_metadata::Dependency) -> bool {
	match dependency.source {
		// This is the criterion cargo uses (in `SourceId::from_url`) to decide
		// whether a dependency has the 'registry' kind.
...
impl Manifests {
	...
	fn get_dependencies(
		...
		// Map the names of user-specified dependencies to the (optionally)
		// requested version.
